...
export const isNextWeekBasketballPostseason = (
  sport: string,
  contents: WeekInfo[],
  week: number
): boolean => {
  ...
  return (
    sport === 'basketball' &&
    contents.some((x) => x.week === nextWeek && x.postseasonInd)
  );
};

export const isNextWeekBowlGameWeek = (
  sport: string,
  contents: WeekInfo[],
  week: number
): boolean => {
  const nextWeek = week + 1;
  return (
    sport === 'football' && contents[contents.length - 1].week === nextWeek
  );
};
