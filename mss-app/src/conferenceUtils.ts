...
export const getConferenceCasing = (conference: string) =>
  conferenceCasing.find((x) => x.id === conference);

export const getConferenceCasingBySlug = (conference: string) =>
  conferenceCasing.find((x) => x.slug === conference);

export const getConferenceContractData = (
  conference: string,
  season: string
) => {
  return contractData
...
export const hasBasketballPostseason = (year: string): boolean => {
  return (
    validSportYears.find((validSportYear) => validSportYear.season === year)
      ?.hasPostseason ?? false
  );
};
